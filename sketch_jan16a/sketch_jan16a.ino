...
const char ssid[] ="GEARBOX MEMBERS";//ssid is the name of the wifi network
const char pass[] ="Members@Gearbox";//password of the wifi network
//include mqtt client

#define mqtt_server "192.168.0.241" // ip address of the server
...
void callback(const MQTT::Publish& pub) {
  ...
  else if(payload_received.equalsIgnoreCase("off") ==0){
...
void reconnect(){
  ...
  while(!client.connected()){
    ...
    if(client.connect("My esp")){//if the esp gets connected to mqtt,print 'connected' ... this will be the last iteration of the loop since the esp is now connected
      ...
      client.subscribe("esp1/runmotor");
    }else{
      Serial.print("failed, ");
...
void loop() {
 ...
 //}
  ...
  if(distance >=400 || distance <=2){//the ultrasonic sensor is inaccurate for distances >=400 cm and those <= 2 cm hence out of range
    ...
    Serial.print("cm");
    Serial.println();
   // Serial.println("payload received: " + payload_received);
  }
  delay(1000);//wait for .5 secs before the next iteration
